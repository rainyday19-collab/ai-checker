...
const supportedTypes = { pdf: 'application/pdf', docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg' };
...
function kind(file) {
  ...
  return extension === 'pdf' || extension === 'docx' ? extension : 'image';
}
...
export default function StudentWorkUpload({ files, onFilesChange, disabled = false }) {
  ...
  function add(selectedFiles) {
    ...
    if (kinds.some((value) => value == null)) {
      setError('Unsupported file. Choose one PDF, one DOCX, or PNG/JPG image pages.');
      return;
    }
    const combined = [...files, ...incoming];
    const combinedKinds = combined.map(kind);
    if (combinedKinds.some((value) => value === 'pdf' || value === 'docx') && combined.length > 1) {
      setError(combinedKinds.every((value) => value === 'pdf') ? 'Only one PDF can be graded at a time.'
        : combinedKinds.every((value) => value === 'docx') ? 'Only one DOCX can be graded at a time.'
        : 'PDF and DOCX documents cannot be combined with other files.');
      return;
    }
  ...
  return <div className="student-work-upload">
    ...
    <div className={`upload-area multi-upload-area ${dragging ? 'is-dragging' : ''}`}
      ...
      <span className="upload-description">One PDF or DOCX, or up to 10 ordered PNG/JPG images</span>
      <div className="upload-actions">
        <button className="secondary-button" type="button" disabled={disabled} onClick={() => pickerRef.current?.click()}>Choose files</button>
        <button className="secondary-button" type="button" disabled={disabled || files.some((file) => kind(file) !== 'image')} onClick={() => cameraRef.current?.click()}>Take photo</button>
      </div>
    ...
    {files.length > 0 && <ol className="upload-page-list" aria-label="Selected student work pages" aria-describedby={error ? errorId : undefined}>
      {files.map((file, index) => <li key={`${file.name}-${file.size}-${file.lastModified}-${index}`}>
        <div className="upload-page-details"><strong>{kind(file) === 'pdf' ? 'PDF' : kind(file) === 'docx' ? 'Word document' : `Page ${index + 1}`}</strong><span>{file.name}</span><small>{file.name.split('.').pop()?.toUpperCase()} · {readableSize(file.size)}</small></div>
